...
async function getRecipes() {
  ...
  //Can mark () => functions as async like this
  return new Promise(async (resolve, reject) => {
    ...
    RECIPE_URLS.forEach(async (currVal, ind, arr) => {
      try{
        ...
        // wouldn't use an await in a promise
        // when you call a promise, would use an await
        ...
        const result = await fetch(currVal)
       ...
       // A7. TODO - For each fetch response, retrieve the JSON from it using .json().
        ...
        const newRecipe = await result.json();
        ...
        if(fetchedRecipes.length == arr.length){
          console.log("End ind " + ind + " with length " + arr.length);
          console.log(fetchedRecipes);
          saveRecipesToStorage(fetchedRecipes);
          resolve(fetchedRecipes);
        }else{
          console.log("At ind " + ind + " with length " + arr.length);
        }
